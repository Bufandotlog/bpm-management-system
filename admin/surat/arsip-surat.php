...
if (isset($_GET['set_tgl']) && is_numeric($_GET['set_tgl']) && !empty($_GET['tanggal'])) {
    if (hash_equals($_SESSION['csrf_token'] ?? '', $_GET['csrf_token'] ?? '')) {
        ...
        if (empty($error) && $tgl_db) {
            ...
            if ($res) {
                redirect('admin/surat/arsip-surat.php?jenis=$jenis', 'Tanggal Dikirim berhasil diperbarui untuk seluruh grup.', 'success');
            } else {
...
if (isset($_GET['reset_tgl']) && is_numeric($_GET['reset_tgl'])) {
    if (hash_equals($_SESSION['csrf_token'] ?? '', $_GET['csrf_token'] ?? '')) {
        ...
        if ($res) {
            redirect('admin/surat/arsip-surat.php?jenis=$jenis', 'Tanggal Dikirim berhasil direset untuk seluruh grup.', 'success');
        } else {
