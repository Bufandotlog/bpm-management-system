    ...
    if (!csrfVerify()) {
        ...
        if (empty($nama) || !$tahun_mulai || !$tahun_selesai) {
            ...
            if ($cek) {
                ...
                redirect('admin/system/periode-kepengurusan.php', 'Periode berhasil ditambahkan beserta 6 kementerian default!', 'success');
                exit();
            }
        }
    }
    ...
    if (!csrfVerify()) {
        ...
        if ($id > 0) {
            ...
            try {
                ...
                redirect('admin/system/periode-kepengurusan.php', 'Periode berhasil diaktifkan!', 'success');
                exit();
            } catch (Exception $e) {
                dbRollback();
        ...
        if ($periode_aktif && (int)$periode_aktif['id'] === $id) {
            ...
            if ($periode) {
                ...
                try {
                    ...
                    redirect('admin/system/periode-kepengurusan.php', 'Periode berhasil dihapus!', 'success');
                    exit();
                } catch (Exception $e) {
                    dbRollback();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'edit') {
    if (!csrfVerify()) {
        ...
        if (empty($nama) || !$tahun_mulai || !$tahun_selesai) {
            ...
            redirect('admin/system/periode-kepengurusan.php', 'Periode berhasil diupdate!', 'success');
            exit();
        }
    }
}
