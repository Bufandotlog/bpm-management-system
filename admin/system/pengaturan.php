...
$user = dbFetchOne(
    "SELECT id, username, nama, email, role, periode_id, last_login, created_at,
            totp_enabled, last_ip, google_id, google_email, google_linked_at
     FROM users WHERE id = ?",
    [$admin_id], "i"
);
if (!$user) {
    redirect('admin/auth/login.php', 'Sesi tidak valid.', 'error');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    if (!csrfVerify()) {
        ...
        if (empty($nama)) {
            ...
            redirect('admin/system/pengaturan.php', 'Profil berhasil diperbarui!', 'success');
            exit();
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_password'])) {
    if (!csrfVerify()) {
        ...
        if (isRateLimited('pwd_change_failed', 5, 15, (string)$admin_id)) {
            $msg_password = ['type'=>'error','text'=>'Terlalu banyak percobaan gagal ganti password. Coba lagi dalam 15 menit.'];
        } elseif (!$freshUser || !password_verify($current, $freshUser['password'])) {
            recordFailedAttempt('pwd_change_failed', (string)$admin_id);
            ...
            redirect('admin/system/pengaturan.php', 'Password berhasil diubah!', 'success');
            exit();
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['disable_2fa'])) {
    if (!csrfVerify()) {
        ...
        if (!$pwRow || !password_verify($current, $pwRow['password'])) {
            ...
            redirect('admin/system/pengaturan.php', '2FA berhasil dinonaktifkan.', 'success');
            exit();
        }
    }
}
...
<div class="settings-card">
    ...
    <div class="twofa-actions">
        <?php if (!$totpEnabled): ?>
            <a href="<?php echo baseUrl('admin/auth/2fa-setup.php'); ?>" class="btn-primary">
                <i class="fas fa-qrcode"></i> Setup 2FA Sekarang
            </a>
        <?php else: ?>
            <a href="<?php echo baseUrl('admin/auth/2fa-setup.php'); ?>" class="btn-secondary">
...
<!-- TAUTAN AKUN GOOGLE -->
<div class="settings-card">
    <h2><i class="fab fa-google"></i> Penautan Akun Google</h2>
    <p style="color:#aaa;font-size:.9rem;margin-bottom:1rem;">
        Tautkan akun Google untuk mempercepat proses login tanpa harus mengetik password.
    </p>
    <?php
    $googleLinked = !empty($user['google_id']);
    $googleEmail  = $user['google_email'] ?? '';
    $googleLinkedAt = !empty($user['google_linked_at']) ? date('d/m/Y H:i', strtotime($user['google_linked_at'])) . ' WIB' : '-';
    $googleConfig = getGoogleAuthConfig();
    ?>
    <div class="twofa-status <?php echo $googleLinked ? 'twofa-on' : 'twofa-off'; ?>" style="margin-bottom:1rem;">
        <i class="fab fa-google" style="color: <?php echo $googleLinked ? '#4285F4' : '#888'; ?>; font-size: 1.5rem;"></i>
        <div>
            <strong><?php echo $googleLinked ? 'Google Terhubung' : 'Google Belum Terhubung'; ?></strong>
            <div style="font-size:.85rem;opacity:.85;margin-top:2px;">
                <?php if ($googleLinked): ?>
                    Terkoneksi ke: <strong><?php echo htmlspecialchars($googleEmail); ?></strong> (<?php echo $googleLinkedAt; ?>)
                <?php else: ?>
                    Tautkan akun Google kamu untuk mengaktifkan fitur Quick Sign-In.
                <?php endif; ?>
            </div>
        </div>
    </div>
    <div class="twofa-actions">
        <?php if (!$googleLinked): ?>
            <?php if ($googleConfig['configured']): ?>
                <a href="<?php echo baseUrl('admin/auth/google-auth.php?action=link'); ?>" class="btn-primary" style="display:inline-flex;align-items:center;gap:8px;text-decoration:none;background:linear-gradient(135deg, #4285F4 0%, #34A853 100%);">
                    <i class="fab fa-google"></i> Tautkan Akun Google
                </a>
            <?php else: ?>
                <div style="color:#ffaa00;font-size:0.85rem;padding:8px;background:rgba(255,170,0,0.1);border-radius:6px;">
                    <i class="fas fa-exclamation-triangle"></i> Konfigurasi Google OAuth (Client ID & Secret) belum diisi pada file .env server.
                </div>
            <?php endif; ?>
        <?php else: ?>
            <form method="POST" action="<?php echo baseUrl('admin/auth/google-auth.php'); ?>" style="display:inline;">
                <?php echo csrfField(); ?>
                <input type="hidden" name="action" value="unlink">
                <button type="submit" class="btn-danger" onclick="return confirm('Apakah Anda yakin ingin melepas tautan akun Google ini?')" style="display:inline-flex;align-items:center;gap:8px;">
                    <i class="fas fa-unlink"></i> Lepas Tautan Google
                </button>
            </form>
        <?php endif; ?>
    </div>
