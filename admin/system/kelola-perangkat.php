...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_putus'])) {
    if (!csrfVerify()) {
        redirect('admin/system/kelola-perangkat.php', 'Request tidak valid.', 'error');
        exit();
    }
    ...
    if ($putusId > 0) {
        ...
        if ($cek) {
            ...
            redirect('admin/system/kelola-perangkat.php', 'Sesi berhasil diputuskan.', 'success');
        } else {
            redirect('admin/system/kelola-perangkat.php', 'Sesi tidak ditemukan.', 'error');
        }
    }
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_putus_semua'])) {
    ...
    redirect('admin/system/kelola-perangkat.php', 'Semua sesi lain berhasil diputuskan.', 'success');
    exit();
}
