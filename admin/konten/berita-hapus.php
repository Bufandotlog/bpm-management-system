...
$id = (int) ($_GET['id'] ?? 0);
if (!$id) {
    redirect('admin/konten/berita.php', 'ID berita tidak valid', 'error');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrfVerify()) {
        redirect('admin/konten/berita.php', 'Request tidak valid.', 'error');
        exit();
    }
    if (($_POST['confirm'] ?? '') === 'yes') {
        ...
        redirect('admin/konten/berita.php', 'Berita berhasil dihapus!', 'success');
    } else {
        redirect('admin/konten/berita.php', 'Penghapusan dibatalkan', 'info');
    }
    exit();
}
?>
...
<link rel="stylesheet" href="<?php echo baseUrl('admin/css/berita-hapus.css'); ?>">
