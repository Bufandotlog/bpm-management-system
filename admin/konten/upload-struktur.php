...
echo '<link rel="stylesheet" href="' . baseUrl('admin/css/upload-struktur.css') . '?v=' . (file_exists(__DIR__ . '/../css/upload-struktur.css') ? filemtime(__DIR__ . '/../css/upload-struktur.css') : '1') . '">';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_hapus_gambar'])) {
    if (!csrfVerify()) {
        redirect('admin/konten/upload-struktur.php', 'Request tidak valid.', 'error');
        exit();
    }
    ...
    redirect('admin/konten/upload-struktur.php', 'Gambar berhasil dihapus.', 'success');
    exit();
}
    ...
    if (!csrfVerify()) {
        ...
        if ($upload_result) {
            ...
            try {
                ...
                redirect('admin/konten/upload-struktur.php', 'Gambar struktur organisasi berhasil diupload!', 'success');
                exit();
            } catch (Exception $e) {
                dbRollback();
