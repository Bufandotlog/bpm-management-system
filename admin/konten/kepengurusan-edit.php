...
if (!in_array($posisi, $posisi_valid)) {
    redirect('admin/konten/kepengurusan.php', 'Posisi tidak valid', 'error');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_hapus'])) {
    if (!csrfVerify()) {
        redirect('admin/konten/kepengurusan-edit.php?posisi=' . urlencode($posisi), 'Request tidak valid.', 'error');
        exit();
    }
    ...
    if ($action === 'hapus_foto' && $data && !empty($data['foto'])) {
        ...
        redirect('admin/konten/kepengurusan-edit.php?posisi=' . urlencode($posisi), 'Foto berhasil dihapus', 'success');
        exit();
    }
    ...
    if ($action === 'hapus_logo' && $data && !empty($data['logo'])) {
        ...
        dbQuery("UPDATE struktur_bph SET logo = NULL WHERE id = ? AND periode_id = ?",
                [$data['id'], $active_periode], "ii");
        redirect('admin/konten/kepengurusan-edit.php?posisi=' . urlencode($posisi), 'Logo berhasil dihapus', 'success');
        exit();
    }
    ...
    if ($action === 'hapus_foto_anggota' && isset($_POST['anggota_id'])) {
        ...
        redirect('admin/konten/kepengurusan-edit.php?posisi=' . urlencode($posisi), 'Foto anggota berhasil dihapus', 'success');
        exit();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['action_hapus'])) {
    ...
    try {
        ...
        redirect('admin/konten/kepengurusan.php', 'Data berhasil disimpan!', 'success');

    } catch (Exception $e) {
        dbRollback();
        error_log("[KEPENGURUSAN-EDIT] " . $e->getMessage());
        redirect('admin/konten/kepengurusan-edit.php?posisi=' . urlencode($posisi),
...
<link rel="stylesheet" href="<?php echo baseUrl('admin/css/kepengurusan-edit.css'); ?>">
