...
$dbRateLimited  = isRateLimited('2fa_failed', 5, 15, (string)$userId);
$isLocked       = ($locked2faUntil > 0 && time() < $locked2faUntil) || $dbRateLimited;

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrfVerify()) {
        $error = 'Request tidak valid.';
    } elseif ($isLocked) {
        $error = 'Terlalu banyak percobaan gagal 2FA. Coba lagi dalam 15 menit.';
    } else {
        ...
        if (strlen($code) !== 6) {
            ...
            if (totpVerifyWithReplay($user['totp_secret'], $code, $userId)) {
                ...
                // Kode salah
                recordFailedAttempt('2fa_failed', (string)$userId);
                $n = $attempts2fa + 1;
