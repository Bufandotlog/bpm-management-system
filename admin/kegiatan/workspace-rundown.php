...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_arsip') {
    if (!csrfVerify()) {
        ...
        $nama_acara    = $kegiatan['nama_kegiatan'];
...
<div class="cetak-rundown-container">
    ...
    <?php if ($success_msg): ?>
        <div style="background: rgba(46, 204, 113, 0.1); color: #2ecc71; padding: 15px; border-radius: 12px; border: 1px solid rgba(46, 204, 113, 0.2); margin-bottom: 20px;">
            ...
            <a href="<?php echo baseUrl('admin/rundown/arsip-rundown.php'); ?>" style="color: #4facfe; margin-left: 10px; text-decoration: underline;">Lihat Arsip →</a>
        </div>
    <?php endif; ?>
    <?php if ($error_msg): ?>
    ...
    <form method="POST" id="rundownForm">
        ...
        <div class="card">
            <div class="card-header">
                ...
                <h2><?php echo $edit_id > 0 ? 'Edit Rundown' : 'Workspace: Susunan Acara (Sie Acara)'; ?></h2>
                <?php if ($edit_id > 0): ?>
        ...
        <div class="actions-bar">
            <div style="display: flex; gap: 12px; align-items: center; width: 100%; justify-content: flex-end;">
                <?php if ($edit_id > 0): ?>
                    <a href="workspace-rundown.php?kegiatan_id=<?php echo $kegiatan_id; ?>" style="color: #888; text-decoration: none; padding: 14px 20px; border-radius: 12px; font-weight: 600; border: 1px solid var(--border-color); transition: 0.3s;">
...
function submitPrint() {
    ...
    clonedForm.action = '<?php echo baseUrl("admin/rundown/cetak-rundown-pdf.php"); ?>';
