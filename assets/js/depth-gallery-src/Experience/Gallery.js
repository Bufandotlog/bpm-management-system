...
class Gallery {
  constructor(debug = null) {
    ...
    this.debug = debug
    this.scope = document.querySelector('[data-depth-gallery-viewport]')
  ...
  setPlanes(scene) {
    ...
    this.planeConfig.forEach((plane, index) => {
      ...
      planeMesh.userData.title = plane.title || labelData.word || `Card ${index + 1}`
      planeMesh.userData.subtitle =
        plane.subtitle || `Card ${String(index + 1).padStart(2, '0')}`
  ...
  updatePlaneScale() {
    ...
    const viewportScale = THREE.MathUtils.clamp(
      Math.min(window.innerWidth / 1000, window.innerHeight / 600),
      0.78,
      1
    )
    const scale = isMobileViewport
      ? this.mobilePlaneScale
      : this.desktopPlaneScale * viewportScale
  ...
  updatePlaneVisibility(cameraZ) {
    ...
    this.planes.forEach((plane, index) => {
      ...
      const currentOpacity = Number.isFinite(plane.userData.transitionOpacity)
        ? plane.userData.transitionOpacity
        : Number.isFinite(plane.material.opacity)
          ? plane.material.opacity
          : 0
      const transitionOpacity = THREE.MathUtils.lerp(
        currentOpacity,
        targetOpacity,
        this.planeFadeSmoothing
      )
      plane.userData.transitionOpacity = transitionOpacity
      plane.material.opacity = transitionOpacity
  ...
  bindPointerEvents() {
    ...
    this.scope.addEventListener('pointerleave', this.onPointerLeave, { passive: true })
  }
