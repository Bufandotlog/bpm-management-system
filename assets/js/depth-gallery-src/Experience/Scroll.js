...
class Scroll {
  constructor(camera, gallery, debug = null, scope = null) {
    ...
    this.sectionStart = 0
    this.onPageScroll = () => {
      const travel = Math.max(this.scope.offsetHeight - window.innerHeight, 1)
      const progress = THREE.MathUtils.clamp(
        (window.scrollY - this.sectionStart) / travel,
        0,
        1
      )
      const cameraRange = Math.max(this.maxCameraZ - this.minCameraZ, 0)
      this.scrollTarget = progress * cameraRange / Math.max(this.scrollToWorldFactor, 0.0001)
    }
    ...
    this.onWheel = (event) => {
      ...
      const atStart = this.camera.position.z >= this.maxCameraZ - 0.01
      const atEnd = this.camera.position.z <= this.minCameraZ + 0.01
      if ((normalizedWheelDelta < 0 && atStart) || (normalizedWheelDelta > 0 && atEnd)) return
    ...
    this.onTouchMove = (event) => {
      ...
      if (
        (deltaY < 0 && this.camera.position.z >= this.maxCameraZ - 0.01) ||
        (deltaY > 0 && this.camera.position.z <= this.minCameraZ + 0.01)
      ) return
      event.preventDefault()
  ...
  init() {
    ...
    this.sectionStart = this.scope.getBoundingClientRect().top + window.scrollY
    this.scrollTarget = 0
    this.scrollCurrent = 0
